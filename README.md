...
La complejidad implícita puede describirse como lineal `O(n)` para toda cadena `ω` procesada por lo que Sudkamp (2006) introduce como una "máquina de estado finito" (si se quiere ver así al DFA modelado, también pudiérase interpretar de esta manera a la expresión regular planteada). (p.147)
